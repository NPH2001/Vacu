...
import { getProductGallery, getProductReviews } from '@/lib/data';

export default async function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const [rows, cats, farms, gallery, reviews] = await Promise.all([
  ...
  return (
    <div className="space-y-5">
      ...
      <ProductForm action={boundUpdate} defaults={row} categories={cats} farmers={farms} editing gallery={gallery} reviews={reviews} />
    </div>
  );
}
